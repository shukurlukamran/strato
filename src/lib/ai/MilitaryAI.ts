...
export class MilitaryAI {
  ...
  async decideActions(
    ...
    if (!bans.banRecruitment && recruitAmount > 0) {
      // Use ActionPricing for consistent cost calculation with resource penalties
      ...
      if (finalRecruitAmount > 0) {
        const pricingResult = ActionPricing.calculateRecruitmentPricing(finalRecruitAmount, stats);

        // Check both budget and resource affordability
        if (pricingResult.cost <= remainingBudget && pricingResult.resourceCostInfo.canAfford) {
          actions.push({
